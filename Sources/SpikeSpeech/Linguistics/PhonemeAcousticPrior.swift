...
public final class PhonemeAcousticPrior: @unchecked Sendable {
    ...
    public init(
        ...
        while pId < vocabSize {
            ...
            // 音素固有のフォルマント共鳴パラメータ定義
            switch pId {
            case 5: // /a/ (あ): F1=800, F2=1300, F3=2600
                Self.synthesizeFormantSpectrum(
                    powerSpec: &powerSpec,
                    formants: [(800.0, 90.0, 1.0), (1300.0, 110.0, 0.6), (2600.0, 160.0, 0.18)],
                    maxFreq: maxFreq,
                    fftBins: fftBins,
                    tiltDecay: 0.85
                )
            case 6: // /i/ (い): F1=300, F2=2300, F3=3000
                Self.synthesizeFormantSpectrum(
                    powerSpec: &powerSpec,
                    formants: [(300.0, 70.0, 0.9), (2300.0, 130.0, 0.5), (3000.0, 180.0, 0.18)],
                    maxFreq: maxFreq,
                    fftBins: fftBins,
                    tiltDecay: 0.85
                )
            case 7: // /u/ (う): F1=360, F2=1200, F3=2400
                Self.synthesizeFormantSpectrum(
                    powerSpec: &powerSpec,
                    formants: [(360.0, 70.0, 0.85), (1200.0, 100.0, 0.4), (2400.0, 150.0, 0.12)],
                    maxFreq: maxFreq,
                    fftBins: fftBins,
                    tiltDecay: 0.85
                )
            case 8: // /e/ (え): F1=500, F2=1900, F3=2600
                Self.synthesizeFormantSpectrum(
                    powerSpec: &powerSpec,
                    formants: [(500.0, 80.0, 0.95), (1900.0, 120.0, 0.5), (2600.0, 160.0, 0.18)],
                    maxFreq: maxFreq,
                    fftBins: fftBins,
                    tiltDecay: 0.85
                )
            case 9: // /o/ (お): F1=500, F2=900, F3=2500
                Self.synthesizeFormantSpectrum(
                    powerSpec: &powerSpec,
                    formants: [(500.0, 80.0, 0.95), (900.0, 90.0, 0.65), (2500.0, 150.0, 0.12)],
                    maxFreq: maxFreq,
                    fftBins: fftBins,
                    tiltDecay: 0.85
                )
            ...
            case 13, 15, 24: // 鼻音 (n, m, N): 低域共鳴集中
                Self.synthesizeFormantSpectrum(
                    powerSpec: &powerSpec,
                    formants: [(250.0, 100.0, 0.7), (1000.0, 200.0, 0.15)],
                    maxFreq: maxFreq,
                    fftBins: fftBins,
                    tiltDecay: 0.7
                )
            case 16, 17, 18, 19, 20, 21, 22, 23, 26, 30, 31, 32, 33, 34, 35, 36, 37, 38: // 有声子音・拗音・長音: 一般有声共鳴
                Self.synthesizeFormantSpectrum(
                    powerSpec: &powerSpec,
                    formants: [(450.0, 120.0, 0.6), (1600.0, 180.0, 0.25)],
                    maxFreq: maxFreq,
                    fftBins: fftBins,
                    tiltDecay: 0.8
                )
    ...
    private static func synthesizeFormantSpectrum(
        ...
        while b < fftBins {
            ...
            while fIdx < formants.count {
                ...
                // Cauchy-Lorentz 共鳴形状
                let diff = f - f0
                let denom = 1.0 + ((diff * diff) / ((bw * 0.5) * (bw * 0.5)))
                resSum += g / denom
                fIdx += 1
            }
